    ...
    public class MasterDataSvc : IMasterDataSvc
    {
        ...
        public async Task InitializeCacheData()
        {
            ...
            try
            {
                ...
                // Products
                await _baseDb.KeyDeleteAsync(CacheConst.ALL_PRODUCT_HASHKEY);
        ...
        public async Task<Dictionary<string, StockItemKey>> GetProductMap(bool reload = false)
        {
            ...
            try
            {
                if (!reload)
                {
                    var itemsArray = await _baseDb.HashGetAllAsync(CacheConst.ALL_PRODUCT_HASHKEY);
                ...
                if (reload || itemMap.Count == 0)
                {
                    ...
                    foreach (var stockItemKey in dbResult)
                    {
                        ...
                        if (!await _baseDb.SetHashValueAsync(CacheConst.ALL_PRODUCT_HASHKEY, _hashKey_, stockItemKey))
                        {
